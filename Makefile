...
install:  ## Checks and installs dependencies
	...
	@docker-compose run --rm follow_me bash -c "bundle check || bundle install"

start: ## Starts the development server
	$(info Starting the development server)
	@docker-compose run --rm --service-ports follow_me bundle exec rails s -b 0.0.0.0

test_setup: install ## Setup the test environment
	$(info Setting up the test environment)
	@docker-compose run --rm follow_me bash -c "RAILS_ENV=test rake db:drop db:create && RAILS_ENV=test rake environment db:schema:load"
